...
import { contactBody, groupBody, groupData, type PropsForMenuChat } from '@/components/types'
      ...
      <div className="w-full h-full grid grid-cols-1 bg-zinc-900 menu-chat-area gap-2 ">
        ...
        <div className="w-full flex flex-col [grid-area:channels]  justify-start items-start gap-2">
          <div className="w-full h-16 flex flex-row justify-between items-center border-b-4 pl-7 pr-5 border-zinc-950 shadow drop-shadow-xl" >
            ...
            <button className="w-6 h-6 flex flex-col justify-center items-center rounded-md bg-zinc-700" onClick={props.OpenCardToSelectOptions} data-testid='MoreActionListButton' >{iconsForChatsPage[0].icon}</button>
          </div>
          ...
          <div className="w-full flex flex-col justify-between items-start gap-7 pl-7 pr-5 overflow-hidden ">
            ...
            <div key={userData.user_id} className="w-full h-32 flex flex-col justify-start items-start gap-2 overflow-y-auto scroll-bar">
              {groupChatsList.length > 0 ? (<>{
                ...
                :
                (<><div className='flex flex-col justify-center items-center gap-4 self-center '><div data-testid="ThereArentGroupsIconDiv">{iconsForChatsPage[13].icon}</div>
                  <p >There aren't Groups</p>
                </div></>)
              }
            ...
            <div key={userData.user_id} className="w-full h-32 flex flex-col  justify-start items-start gap-3  overflow-y-auto scroll-bar ">
              {
                contactsList.length > 0 ? (<>
                  ...
                  :
                  (<><div className='flex flex-col justify-center items-center gap-4 self-center '><div data-testid="ThereArentContactsIconDiv">{iconsForChatsPage[13].icon}</div>
                    <p >There aren't Contacts</p>
                  </div></>)
        ...
        <div className='w-full h-20 flex flex-row gap-4  items-center [grid-area:Navigation] self-end pl-7 pr-5' data-testid='optionsListButton' onClick={props.OpenNavigationMenuHandler} >
          {!userData.profile_picture ? (<>
            <div className="animate-pulse w-[40px] h-[40px] rounded-xl bg-zinc-200"></div>
          </>) : (<>
            <Image data-testid='profilePicture' loading="eager" src={userData.profile_picture} className='rounded-lg' width="40" height="40" alt='profileImage' />
          </>)
          }
          <p className=" md:flex md:flex-col md:justify-center">{userData.username}</p>
          <button className="hidden md:flex-1 md:flex md:flex-col md:justify-center-end md:items-end">{
          ...
          }</button>
          {
            ...
            <ul className='w-56 absolute bottom-20 left-9 flex flex-col justify-between items-center gap-1 bg-zinc-800 border-zinc-700 border-2 rounded-xl p-4' data-testid="navBarListOptions">
              {
                infoForNavMenuChat.map((elem) => {
                  return elem.funtionLogOut ?
                    <li onClick={props.LogOutHandler} key={elem.id} data-testid={elem.dataTestId} className={elem.class}>{elem.icon}<span onClick={props.LogOutHandler}>{elem.name}</span></li>
                    :
                    <Link key={elem.id} href={elem.link} className='w-full' data-testid={elem.dataTestId} ><li className={elem.class}>{elem.icon}<span>{elem.name}</span></li></Link>
                })
              }
            </ul>
          }
