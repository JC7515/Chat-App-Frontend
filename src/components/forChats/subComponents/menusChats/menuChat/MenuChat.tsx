...
import { type PropsForMenuChat } from '@/components/types'
...
const MenuChat: React.FC<Props> = ({ props }) => {
  ...
  return (
    <>
      <div className={`${props.menuOpen && !props.isOpenAChat || props.userData.chat_type === CONTACT_CHAT && screen.width > 768 || props.userData.chat_type === '' && screen.width > 768 ? "" : "hidden"}  w-[282px] h-full bg-zinc-900 text-white absolute z-30  md:relative md:w-96 lg:w-96`} >
              ...
              <div className="w-full h-32 flex flex-col justify-start items-start gap-2 overflow-y-auto scroll-bar">
                {
                  groupChatsList.map(({ group, notifications_number }: any) => {
                    return <div id={group.group_id} key={group.group_id} className="w-full py-3 pr-4 flex flex-row justify-start items-center hover:pl-2 gap-3 hover:bg-zinc-700 rounded-lg" onClick={props.CloseMenuOpenGroupChatHandler}>
              ...
              <div className="w-full h-32 flex flex-col  justify-start items-start gap-3  overflow-y-auto scroll-bar ">
                {
                  contactsList.map((contact: any) => {
                    return <div key={contact.chat_id} id={contact.chat_id} className="w-full py-2 pr-4 flex flex-row items-center hover:pl-2 gap-3 hover:bg-zinc-700 rounded-lg" onClick={props.CloseMenuOpenContactChatHandler}>
